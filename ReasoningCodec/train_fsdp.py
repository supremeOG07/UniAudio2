...
Target: A Flow-matching based vocoder that transfer the tokens into scalar latent representation
...
from data.dataloader_cfm_rec import get_data_iterator_tokenizer_vocabulary
...
from models.model_utils import load_state, verify_state
...
def get_args():
    ...
    parser.add_argument('--segment_duration', type=int, default=20, help="the segment duration for training. 20s is the default value")
    ...
    parser.add_argument('--reason_lm_path', type=str, default='', help='the path for audio reasoning model')
    parser.add_argument('--reconstruction_path', type=str, default='', help='the path for reconstruction model')
    parser.add_argument('--llm_path', type=str, default='', help='the path for llama model')
    parser.add_argument('--prompt_path', type=str, default='', help='the path for prompt model')
    parser.add_argument('--best_rq_ckpt', type=str, default='', help='the path of BESTRQ model')
...
def main():
    ...
    model = AudioDiffusion1D(
        ...
        reason_lm_path = args.reason_lm_path,
        best_rq_ckpt = args.best_rq_ckpt,
    ...
    if args.reconstruction_path is not None:
        load_state(model, args.reconstruction_path)
...
def train_one_epoch(args, model, epoch, train_dl, optimizer, scheduler, reporter, parent_reporter, SQCodec, writer):
    ...
    for b_idx, batch in enumerate(reporter.measure_iter_time(train_dl, "iter_time"), 1):
        ...
        tasks = None
        audios = audios.to(torch.cuda.current_device())
        # audios = audios[:,:,:args.segment_duration*24000] # control the max size
        sq_frame_rate = 25 # 
        ...
        with torch.cuda.amp.autocast(enabled=False):
            with reporter.measure_time("forward_time"):
                ...
                for binx in range(len(durations)):
                    true_latent_mask[binx, 0:int(durations[binx] * sq_frame_rate)] = 2
                
                loss_flow, rec_loss, commitment_loss  = model(input_audios=audios, spectrograms=spectrograms, latents=true_latent, 
                                                    latent_masks=true_latent_mask, tasks=tasks, texts=texts)
                loss = loss_flow + rec_loss*0.1 + commitment_loss*0.5
                loss = loss / args.grad_accum
                metrics = {'loss_flow': loss_flow.clone().detach()} # 'codebook_loss': codebook_loss.clone().detach()
